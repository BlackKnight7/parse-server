  ...
  promise = promise.then(function () {
    ...
    if (!idMatch) {
      ...
      if (deviceTokenMatches.length == 1 && !deviceTokenMatches[0]['installationId']) {
        ...
        return _this9.config.database.destroy('_Installation', _delQuery).then(function () {
          return deviceTokenMatches[0]['objectId'];
        });
      } else {
        if (_this9.data.deviceToken && idMatch.deviceToken != _this9.data.deviceToken) {
          ...
          if (_this9.data.appIdentifier) {
            _delQuery2['appIdentifier'] = _this9.data.appIdentifier;
          }
          _this9.config.database.destroy('_Installation', _delQuery2);
        }
